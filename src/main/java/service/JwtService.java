...
import static utils.SwedishDateTime.now;
...
public class JwtService {
    ...
    private JWTVerifier buildVerifier() {
        ...
        return verification.build(() -> Date.from(localDateTimeToInstant(now())));
    }
    ...
    public String createTokenFromPlayerName(String playerName) {
        ...
        return AUTHORIZATION_PREFIX + jwtBuilder
                .withExpiresAt(Date.from(localDateTimeToInstant(now().plusHours(TOKEN_VALID_HOURS))))
                .sign(algorithm);
    }

    private static Instant localDateTimeToInstant(LocalDateTime time) {
